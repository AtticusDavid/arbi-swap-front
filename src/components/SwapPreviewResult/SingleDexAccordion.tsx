        ...
        <AccordionPanel pb={4} paddingLeft={2}>
          <SkeletonText
            ...
            <UnorderedList spacing={2}>
              {singleDexResult.sort((a,b)=>new Decimal(b.expectedAmountOut).comparedTo(a.expectedAmountOut)).map((item) => <ListItem key={item.dexId}>
                <HStack justifyContent="space-between">
                  <Text fontSize={['sm', 'md', 'md', 'md']}>{item.dexId}</Text>
                  <Text fontSize={['sm', 'md', 'md', 'md']}>{`${(withComma(getTokenOutDenom(item.expectedAmountOut), 3))} ${tokenOut?.symbol}`}</Text>
                </HStack>
              </ListItem>)}
            </UnorderedList>
          </SkeletonText>
