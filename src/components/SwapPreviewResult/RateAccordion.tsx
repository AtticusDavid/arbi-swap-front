...
const RateAccordion = ({
  ...
  const rateText = `1 ${fromTokenSymbol} = ${withComma(outputAmount / inputAmount, 3)} ${toTokenSymbol}`;
        ...
        <AccordionPanel>
          <SkeletonText
            ...
            <HStack justifyContent="space-between">
              ...
              <Text fontSize={['sm', 'md', 'md', 'md']}>{`${withComma(outputAmount / inputAmount, 3)} ${toTokenSymbol}`}</Text>
            </HStack>
            <HStack justifyContent="space-between" mt={2}>
              <Text fontSize={['sm', 'md', 'md', 'md']}>{`1 ${toTokenSymbol} price`}</Text>
              <Text fontSize={['sm', 'md', 'md', 'md']}>{`${(withComma(inputAmount / outputAmount, 3))} ${fromTokenSymbol}`}</Text>
            </HStack>
          </SkeletonText>
        </AccordionPanel>
      </AccordionItem>
